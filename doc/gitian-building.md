...
*Setup instructions for a gitian build of ZARCASH using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the ZARCASH
...
- [Building ALQO](#building-zarcash)
...
Clone the git repositories for zarcash and gitian and then checkout the zarcash version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the zarcash repository
...
gitian builds' in [doc/release-process.md](release-process.md) in the zarcash repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/zarcash/.git/
...
./bin/gbuild --commit zarcash=${COMMIT} --url zarcash=${URL} ../zarcash/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit zarcash=${COMMIT} --url zarcash=${URL} ../zarcash/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit zarcash=${COMMIT} --url zarcash=${URL} ../zarcash/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/zarcash-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/zarcash-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/zarcash-build.assert
```
...
[zarcash/gitian.sigs](https://github.com/zarcash-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
