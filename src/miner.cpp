...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("ZARCASHMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("ZARCASHMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("ZARCASHMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("zarcash-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("ZARCASHMiner(): Signing new block failed \n");
                continue;
            }
        ...
        LogPrintf("Running ZARCASHMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ThreadZARCASHMiner() exception");
    } catch (...) {
        LogPrintf("ThreadZARCASHMiner() exception");
    }

    LogPrintf("ThreadZARCASHMiner exiting\n");
}
