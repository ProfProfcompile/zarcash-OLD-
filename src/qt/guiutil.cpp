...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a ZARCASH address (e.g. %1)").arg("GVXmy14r8hmuzt1d5ygnqzZfbuomg1XLmF"));
#endif
...
{
    // return if URI is not valid or is no ZARCASH: URI
...
{
    // Convert zarcash:// to zarcash:
    //
    //    Cannot handle this later, because zarcash:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open zarcash.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "ZARCASH.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for ZARCASH.lnk
...
{
    return GetAutostartDir() / "zarcash.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a zarcash.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=ZARCASH\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the zarcash app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add zarcash app to startup item list
