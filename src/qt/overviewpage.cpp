...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("ZARCASH")
    updateDisplayUnit();
}
...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeZARCASHAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeZARCASHAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeZARCASHAmount = strAnonymizeZARCASHAmount.remove(strAnonymizeZARCASHAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeZARCASHAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeZARCASHAmount * COIN) nMaxToAnonymize = nAnonymizeZARCASHAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeZARCASHAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeZARCASHAmount));
        strAnonymizeZARCASHAmount = strAnonymizeZARCASHAmount.remove(strAnonymizeZARCASHAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeZARCASHAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
